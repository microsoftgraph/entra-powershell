...
BeforeAll {  
    ...
    Mock -CommandName Remove-MgBetaPolicyFeatureRolloutPolicyApplyToByRef -MockWith {} -ModuleName Microsoft.Graph.Entra.Beta.DirectoryManagement
}
...
Describe "Remove-EntraBetaFeatureRolloutPolicyDirectoryObject" {
    Context "Test for Remove-EntraBetaFeatureRolloutPolicyDirectoryObject" {
        It "Should removes a group from the cloud authentication roll-out policy from Azure AD" {
            ...
            Should -Invoke -CommandName Remove-MgBetaPolicyFeatureRolloutPolicyApplyToByRef -ModuleName Microsoft.Graph.Entra.Beta.DirectoryManagement -Times 1
        }
        ...
        It "Should contain DirectoryObjectId in parameters when passed ObjectId to it" {
            Mock -CommandName Remove-MgBetaPolicyFeatureRolloutPolicyApplyToByRef -MockWith {$args} -ModuleName Microsoft.Graph.Entra.Beta.DirectoryManagement
        ...
        It "Should contain 'User-Agent' header" {
            ...
            $result | Should -BeNullOrEmpty
            $userAgentHeaderValue = "PowerShell/$psVersion EntraPowershell/$entraVersion Remove-EntraBetaFeatureRolloutPolicyDirectoryObject"
            Should -Invoke -CommandName Remove-MgBetaPolicyFeatureRolloutPolicyApplyToByRef -ModuleName Microsoft.Graph.Entra.Beta.DirectoryManagement -Times 1 -ParameterFilter {
