...
BeforeAll{
    ...
    $module = Get-Module -Name Microsoft.Graph.Entra.DirectoryManagement
}
                        ...
                        if($null -ne $filter){                             
                            try { 
                                ...
                                if($filter.IsApi){                                     
                                    ...
                                    Mock -CommandName $filter.TargetName -MockWith {} -ModuleName Microsoft.Graph.Entra.DirectoryManagement    
                                    $result = Invoke-Command -ScriptBlock $commandScriptBlock 
                                    $result | Should -BeNullOrEmpty
                                    Should -Invoke -CommandName $filter.TargetName -ModuleName Microsoft.Graph.Entra.DirectoryManagement -Times 1                                
                                }
                            }
                            catch {
