                    ...
                    if(($params -eq 'Id') -or ($params -is [array] -and $params.count -eq 1 -and $params -eq 'Id')){
                        ...
                        if($null -ne $filter){                             
                            try { 
                                ...
                                if($filter.IsApi){                                     
                                    Mock -CommandName Invoke-GraphRequest -MockWith {} -ModuleName Microsoft.Graph.Entra.Groups
                                    ...
                                    Mock -CommandName $filter.TargetName -MockWith {} -ModuleName Microsoft.Graph.Entra.Groups    
                                    $result = Invoke-Command -ScriptBlock $commandScriptBlock 
                                    $result | Should -BeNullOrEmpty
                                    Should -Invoke -CommandName $filter.TargetName -ModuleName Microsoft.Graph.Entra.Groups -Times 1                                
                                }
                            }
                            catch {
