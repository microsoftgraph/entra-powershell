...
BeforeAll {  
    ...
    Mock -CommandName Get-MgServicePrincipalAppRoleAssignedTo -MockWith $scriptblock -ModuleName Microsoft.Graph.Entra.Applications.Applications
}
...
Describe "Get-EntraServiceAppRoleAssigned" {
    Context "Test for Get-EntraServiceAppRoleAssigned" {
        It "Should return service principal application role assignment." {
            ...
            Should -Invoke -CommandName Get-MgServicePrincipalAppRoleAssignedTo  -ModuleName Microsoft.Graph.Entra.Applications.Applications -Times 1
        }
        ...
        It "Should fail when ServicePrincipalId is invalid" {
            { Get-EntraServicePrincipalAppRoleAssignment -ServicePrincipalId "" } | Should -Throw "Cannot bind argument to parameter 'ServicePrincipalId' because it is an empty string.*"
        }
        ...
        It "Should contain ServicePrincipalId in parameters when passed ObjectId to it" {
            Mock -CommandName Get-MgServicePrincipalAppRoleAssignedTo -MockWith {$args} -ModuleName Microsoft.Graph.Entra.Applications.Applications
        ...
        It "Should contain 'User-Agent' header" {
            ...
            $result | Should -Not -BeNullOrEmpty
            $userAgentHeaderValue = "PowerShell/$psVersion EntraPowershell/$entraVersion Get-EntraServicePrincipalAppRoleAssignment"
            Should -Invoke -CommandName Get-MgServicePrincipalAppRoleAssignedTo -ModuleName Microsoft.Graph.Entra.Applications.Applications -Times 1 -ParameterFilter {
