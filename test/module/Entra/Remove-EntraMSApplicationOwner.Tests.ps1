...
Describe "Remove-EntraMSApplicationOwner"{
    It "Should return empty object" {
        $result = Remove-EntraMSApplicationOwner -ObjectId "3ddd22e7-a150-4bb3-b100-e410dea1cb84" -OwnerId "c13dd34a-492b-4561-b171-40fcce2916c5"
        $result | Should -BeNullOrEmpty
        Should -Invoke -CommandName Remove-MgApplicationOwnerByRef -ModuleName Microsoft.Graph.Entra -Times 1
    }
    It "Should fail when ObjectId is empty" {
        { Remove-EntraMSApplicationOwner -ObjectId "" }
    }
    It "Should fail when -OwnerId is empty" {
        { Remove-EntraMSApplicationOwner -OwnerId "" }
    }
    ...
    It "Should contain 'User-Agent' header" {
        Mock -CommandName Remove-MgApplicationOwnerByRef -MockWith {$args} -ModuleName Microsoft.Graph.Entra
        $userAgentHeaderValue = "PowerShell/$psVersion EntraPowershell/$entraVersion Remove-EntraMSApplicationOwner"
        $result = Remove-EntraMSApplicationOwner -ObjectId "3ddd22e7-a150-4bb3-b100-e410dea1cb84" -OwnerId "c13dd34a-492b-4561-b171-40fcce2916c5"
        $params = Get-Parameters -data $result
