BeforeAll{
    ...
    Import-Module (Join-Path $psscriptroot "..\EntraBetaCmdletsMap.ps1") -Force
                        ...
                        if($null -ne $filter){                             
                            try { 
                                ...
                                if($filter.IsApi){                                     
                                    ...
                                    Mock -CommandName $filter.TargetName -MockWith {} -ModuleName Microsoft.Graph.Entra.Beta    
                                    $result = Invoke-Command -ScriptBlock $commandScriptBlock 
                                    $result | Should -BeNullOrEmpty
                                    Should -Invoke -CommandName $filter.TargetName -ModuleName Microsoft.Graph.Entra.Beta -Times 1                                
                                }
                            }
                            catch {
