...
<#
.SYNOPSIS
    Tests that all EntraBeta modules can be successfully imported in PowerShell 5.1 and PowerShell 7+.

.DESCRIPTION
    This test validates cross-version compatibility by attempting to import each EntraBeta module in both
...
BeforeAll {
    ...
    $installedModules = Get-Module -Name 'Microsoft.Entra.Beta.*' -ListAvailable |
        Select-Object -ExpandProperty Path -Unique
    
    if ($installedModules) {
        $script:EntraBetaModules = $installedModules
    }
    else {
    ...
    $script:AllModules = $script:EntraBetaModules
...
Describe "EntraBeta Module Compatibility Tests" {
    ...
    Context "Module Discovery" {
        ...
        It "Should find expected number of EntraBeta modules" {
            # Expected: 9 EntraBeta modules
            $script:AllModules.Count | Should -BeGreaterThan 5
            Write-Host "  Found $($script:AllModules.Count) EntraBeta modules" -ForegroundColor Cyan
        }
    }
    ...
    Context "Current PowerShell Version Compatibility (PS $($script:CurrentPSVersion))" {
        ...
        It "Should successfully import all EntraBeta modules in current PowerShell version" {
            $failures = @()
            $successes = 0
    ...
    Context "PowerShell 5.1 Compatibility" {
        ...
        It "Should successfully import all EntraBeta modules in PowerShell 5.1" {
            if (-not $script:PS51Available) {
                Set-ItResult -Skipped -Because "PowerShell 5.1 is not available on this system"
            }
            $failures = @()
    ...
    Context "Module Export Validation" {
        ...
        It "Should verify all EntraBeta modules export cmdlets" {
            $failures = @()
            $successes = 0
