...
Describe "Get-EntraBetaPrivilegedRoleAssignmentRequest" {
    Context "Connection and basic behavior" {
        ...
        It "Should return empty when SDK returns none" {
            $result = Get-EntraBetaPrivilegedRoleAssignmentRequest -ProviderId "MockProviderId" -Filter "status eq 'Pending'"
    ...
    Context "Parameter mapping" {
        It "Should pass Filter, ProviderId, and Top to SDK" {
            Get-EntraBetaPrivilegedRoleAssignmentRequest -ProviderId "MockProviderId" -Filter "status eq 'Approved'" -Top 5
            ...
            Should -Invoke -CommandName Get-MgBetaPrivilegedRoleAssignmentRequest -ModuleName Microsoft.Entra.Beta.Governance -Times 1 -ParameterFilter {
                $ProviderId | Should -Be 'MockProviderId'
        ...
        It "Should pass Id as PrivilegedRoleAssignmentRequestId" {
            Get-EntraBetaPrivilegedRoleAssignmentRequest -ProviderId "MockProviderId" -Id "aaaaaaaa-0000-1111-2222-bbbbbbbbbbbb"
        ...
        It "Should pass Property to SDK" {
            Get-EntraBetaPrivilegedRoleAssignmentRequest -ProviderId "MockProviderId" -Property id,assignmentState
    ...
    Context "Headers and output" {
        It "Should include 'User-Agent' header" {
            ...
            Get-EntraBetaPrivilegedRoleAssignmentRequest -ProviderId "MockProviderId" -Filter "status eq 'X'"
        ...
        It "Should add ObjectId alias on results" {
            ...
            $res = Get-EntraBetaPrivilegedRoleAssignmentRequest -ProviderId "MockProviderId" -Id 'req-1'
            $res | Should -Not -BeNullOrEmpty
            $res[0].ObjectId | Should -Be 'req-1'
        }

        It "Should execute with -Debug without throwing" {
            { Get-EntraBetaPrivilegedRoleAssignmentRequest -ProviderId "MockProviderId" -Filter "status eq 'X'" -Debug } | Should -Not -Throw
        }
    }
}
