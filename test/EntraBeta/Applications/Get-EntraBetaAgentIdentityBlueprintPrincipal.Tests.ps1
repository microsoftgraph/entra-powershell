...
Describe "Tests for Get-EntraBetaAgentIdentityBlueprintPrincipal" {
    BeforeAll {
        ...
        $script:userAgentHeaderValue = "PowerShell/$psVersion EntraPowershell/$entraVersion Get-EntraBetaAgentIdentityBlueprintPrincipal"
    }
    ...
    It "Result should not be empty" {
        $result = Get-EntraBetaAgentIdentityBlueprintPrincipal -ServicePrincipalId "sp-id-guid"
    ...
    It "Should use stored service principal ID when not provided" {
        $result = Get-EntraBetaAgentIdentityBlueprintPrincipal
        $result | Should -Not -BeNullOrEmpty
    ...
    It "Should use explicit ID over stored ID" {
        $result = Get-EntraBetaAgentIdentityBlueprintPrincipal -ServicePrincipalId "explicit-sp-id"
        $result | Should -Not -BeNullOrEmpty
        Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Beta.Applications -ParameterFilter {
    ...
    It "Should prompt when no stored ID and no parameter" {
        ...
        Mock -CommandName Read-Host -MockWith { "prompted-sp-id" } -ModuleName Microsoft.Entra.Beta.Applications
        $result = Get-EntraBetaAgentIdentityBlueprintPrincipal
        $result | Should -Not -BeNullOrEmpty
    ...
    It "Should fail when not connected" {
        ...
        { Get-EntraBetaAgentIdentityBlueprintPrincipal -ServicePrincipalId "sp-id-guid" -ErrorAction Stop } | Should -Throw "*Not connected to Microsoft Graph*"
    ...
    It "Should fail when ServicePrincipalId is empty string" {
        { Get-EntraBetaAgentIdentityBlueprintPrincipal -ServicePrincipalId "" } | Should -Throw
    }

    It "Should use correct API endpoint" {
        $result = Get-EntraBetaAgentIdentityBlueprintPrincipal -ServicePrincipalId "sp-id-guid"
        Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Beta.Applications -ParameterFilter {
            $Uri -like "*/beta/servicePrincipals/graph.agentIdentityBlueprintPrincipal/*" -and $Method -eq "GET"
        }
    }
    ...
    It "Should contain 'User-Agent' header" {
        ...
        $result | Should -Not -BeNullOrEmpty
        Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Beta.Applications -ParameterFilter {
            $null -ne $Headers -and $Headers.ContainsKey('User-Agent') -and $Headers['User-Agent'] -like "*EntraPowershell*Get-EntraBetaAgentIdentityBlueprintPrincipal*"
        }
    }
    ...
    It "Should execute successfully without throwing an error" {
        ...
        try {
            { Get-EntraBetaAgentIdentityBlueprintPrincipal -ServicePrincipalId "sp-id-guid" } | Should -Not -Throw
        }
        finally {
