...
BeforeAll{
    ...
    $module = Get-Module -Name Microsoft.Entra.SignIns
}
                        ...
                        if($null -ne $filter){             
                            try { 
                                ...
                                if($filter.IsApi){                     
                                    ...
                                    Mock -CommandName $filter.TargetName -MockWith {} -ModuleName Microsoft.Entra.SignIns    
                                    $result = Invoke-Command -ScriptBlock $commandScriptBlock 
                                    $result | Should -BeNullOrEmpty
                                    Should -Invoke -CommandName $filter.TargetName -ModuleName Microsoft.Entra.SignIns -Times 1                                
                                }
                            }
                            catch {
