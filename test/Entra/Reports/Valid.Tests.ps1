                    ...
                    if(($params -eq 'Id') -or ($params -is [array] -and $params.count -eq 1 -and $params -eq 'Id')){
                        ...
                        if($null -ne $filter){             
                            try { 
                                ...
                                if($filter.IsApi){                     
                                    Mock -CommandName Invoke-GraphRequest -MockWith {} -ModuleName Microsoft.Entra.Reports
                                    ...
                                    Mock -CommandName $filter.TargetName -MockWith {} -ModuleName Microsoft.Entra.Reports    
                                    $result = Invoke-Command -ScriptBlock $commandScriptBlock 
                                    $result | Should -BeNullOrEmpty
                                    Should -Invoke -CommandName $filter.TargetName -ModuleName Microsoft.Entra.Reports -Times 1                                
                                }
                            }
                            catch {
