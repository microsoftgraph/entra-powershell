...
Describe "Tests for Remove-EntraAgentIdentity" {
    BeforeAll {
        ...
        Mock -CommandName Get-EntraContext -MockWith { @{Scopes = @("AgentIdentity.DeleteRestore.All", "AgentIdUser.ReadWrite.All") } } -ModuleName Microsoft.Entra.Applications
        Mock -CommandName Invoke-MgGraphRequest -MockWith $singleAgentIdentity -ModuleName Microsoft.Entra.Applications
    ...
    Context "ByAgentId parameter set" {
        BeforeEach {
            ...
            Mock -CommandName Invoke-MgGraphRequest -MockWith $script:singleAgentIdentity -ModuleName Microsoft.Entra.Applications -ParameterFilter {
                $Method -eq "GET" -and $Uri -like "*/servicePrincipals/microsoft.graph.agentIdentity/*"
            }
            # GET for agent users returns one user
            Mock -CommandName Invoke-MgGraphRequest -MockWith $script:agentUsersForIdentity -ModuleName Microsoft.Entra.Applications -ParameterFilter {
        ...
        It "Should delete an Agent Identity by ID" {
            $result = Remove-EntraAgentIdentity -AgentId "agent-id-guid" -Force
        ...
        It "Should call GET to retrieve the Agent Identity first" {
            Remove-EntraAgentIdentity -AgentId "agent-id-guid" -Force
            Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Applications -ParameterFilter {
                $Method -eq "GET" -and $Uri -like "*/v1.0/servicePrincipals/microsoft.graph.agentIdentity/agent-id-guid*"
            }
        }
        ...
        It "Should delete associated Agent Users before the identity" {
            ...
            Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Applications -ParameterFilter {
                $Method -eq "DELETE" -and $Uri -like "*/v1.0/users/agent-user-1*"
            }
        ...
        It "Should delete the Agent Identity service principal" {
            Remove-EntraAgentIdentity -AgentId "agent-id-guid" -Force
            Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Applications -ParameterFilter {
                $Method -eq "DELETE" -and $Uri -like "*/v1.0/servicePrincipals/agent-id-guid*"
            }
        }
        ...
        It "Should fail when AgentId is empty string" {
            { Remove-EntraAgentIdentity -AgentId "" } | Should -Throw
        }
    }
    ...
    Context "ByBlueprintId parameter set" {
        ...
        It "Should delete all Agent Identities for a Blueprint" {
            $result = Remove-EntraAgentIdentity -AgentIdentityBlueprintId "blueprint-id-guid" -Force
            $result | Should -Not -BeNullOrEmpty
        ...
        It "Should use correct filter endpoint for blueprint query" {
            Remove-EntraAgentIdentity -AgentIdentityBlueprintId "blueprint-id-guid" -Force
            Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Applications -ParameterFilter {
                $Method -eq "GET" -and $Uri -like "*/v1.0/servicePrincipals/microsoft.graph.agentIdentity?*agentIdentityBlueprintId*blueprint-id-guid*"
            }
        }
        ...
        It "Should warn when no Agent Identities found" {
            ...
            $result = Remove-EntraAgentIdentity -AgentIdentityBlueprintId "empty-blueprint-id" -Force 3>&1
            Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Applications -ParameterFilter {
                $Method -eq "DELETE"
            } -Times 0
        }
        ...
        It "Should fail when AgentIdentityBlueprintId is empty string" {
            { Remove-EntraAgentIdentity -AgentIdentityBlueprintId "" } | Should -Throw
        }
    }
    ...
    Context "Error handling" {
        It "Should fail when not connected" {
            ...
            { Remove-EntraAgentIdentity -AgentId "agent-id-guid" -Force -ErrorAction Stop } | Should -Throw "*Not connected to Microsoft Graph*"
        }
        ...
        It "Should handle 404 not found for ByAgentId" {
            ...
            { Remove-EntraAgentIdentity -AgentId "nonexistent-id" -Force -ErrorAction Stop } | Should -Throw
        }

        It "Should handle 404 not found for ByBlueprintId" {
            Mock -CommandName Invoke-MgGraphRequest -MockWith { throw "404 NotFound" } -ModuleName Microsoft.Entra.Applications
            { Remove-EntraAgentIdentity -AgentIdentityBlueprintId "nonexistent-id" -Force -ErrorAction Stop } | Should -Throw
        }
    }
    ...
    Context "Confirmation behavior" {
        ...
        It "Should support WhatIf" {
            $result = Remove-EntraAgentIdentity -AgentId "agent-id-guid" -WhatIf
            Should -Invoke -CommandName Invoke-MgGraphRequest -ModuleName Microsoft.Entra.Applications -ParameterFilter {
                $Method -eq "DELETE"
            } -Times 0
        }
