...
BeforeAll {  
    ...
    Mock -CommandName Get-EntraEnvironment -MockWith {GraphEndpoint="https://graph.microsoft.com"} -ModuleName Microsoft.Entra.Applications
...
Describe "Add-EntraServicePrincipalOwner" {
    Context "Test for Add-EntraServicePrincipalOwner" {
        ...
        It "Should fail when OwnerId is invalid" {
            { Add-EntraServicePrincipalOwner -ServicePrincipalId "aaaaaaaa-0b0b-1c1c-2d2d-333333333333" -OwnerId "" } | Should -Throw "Cannot bind argument to parameter 'OwnerId' because it is an empty string."
        }
