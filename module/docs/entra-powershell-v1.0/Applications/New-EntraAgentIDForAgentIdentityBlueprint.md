---
author: givinalis
description: This article provides details on the New-EntraAgentIDForAgentIdentityBlueprint command.
...
ms.date: 12/17/2024
ms.topic: reference
online version: https://learn.microsoft.com/powershell/module/Microsoft.Entra.Applications/New-EntraAgentIDForAgentIdentityBlueprint
schema: 2.0.0
title: New-EntraAgentIDForAgentIdentityBlueprint
---
...
The Agent Identity Blueprint ID (application object ID). If not provided, the cmdlet uses the stored ID from a previous `New-EntraAgentIdentityBlueprint` call. If no stored ID exists, the cmdlet prompts interactively.
...
The cmdlet relies on the stored Agent Identity Blueprint ID (`$script:CurrentAgentBlueprintId`) set by `New-EntraAgentIdentityBlueprint`. If no blueprint ID is stored, the cmdlet fails with an error.

After creating the Agent Identity, the cmdlet stores the ID and AppId in module-level variables (`$script:CurrentAgentIdentityId`, `$script:CurrentAgentIdentityAppId`) and a global variable (`$global:EntraCurrentAgentIdentityId`) for use by other cmdlets.

The cmdlet uses the v1.0 Microsoft Graph endpoint (`/v1.0/servicePrincipals/Microsoft.Graph.AgentIdentity`) and includes retry logic (up to 10 attempts with 10-second waits) to handle propagation delays.
