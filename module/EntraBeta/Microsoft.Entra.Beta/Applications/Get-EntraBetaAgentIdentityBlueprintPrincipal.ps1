...
function Get-EntraBetaAgentIdentityBlueprintPrincipal {
    [CmdletBinding()]
    param (
        ...
        if (-not $ServicePrincipalId) {
            if ((Test-Path variable:script:CurrentAgentBlueprintServicePrincipalId) -and $script:CurrentAgentBlueprintServicePrincipalId) {
                ...
                if (-not $ServicePrincipalId -or $ServicePrincipalId.Trim() -eq "") {
                    Write-Error "No Service Principal ID provided. Please provide an ID or create one first using New-EntraBetaAgentIdentityBlueprintPrincipal." -ErrorAction Stop
                    return
                }
