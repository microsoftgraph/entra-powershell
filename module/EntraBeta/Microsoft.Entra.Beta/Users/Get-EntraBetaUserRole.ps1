...
function Get-EntraBetaUserRole {
    ...
    PROCESS {
        ...
        try {
            ...
            if ($PSBoundParameters.ContainsKey("All") -and $All) {
                $response = Get-MgBetaUserMemberOfAsDirectoryRole @params -PageSize 999 -Headers $customHeaders
            }
            else {
                $response = Get-MgBetaUserMemberOfAsDirectoryRole @params -Headers $customHeaders
            }
