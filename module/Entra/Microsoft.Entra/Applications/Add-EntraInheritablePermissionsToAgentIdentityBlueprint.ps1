...
function Add-EntraInheritablePermissionsToAgentIdentityBlueprint {
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $false, HelpMessage = "The ID of the Agent Identity Blueprint to add inheritable permissions to.")]
    ...
    process {
        ...
        $apiUrl = "$baseUri/microsoft.graph.agentIdentityBlueprint/$($AgentBlueprintId)/inheritablePermissions"
        $allResults = @()
        $firstIteration = $true
        $continueAdding = $true
