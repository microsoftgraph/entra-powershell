...
function Remove-EntraAgentIdentity {
    ...
    process {
        ...
        try {
            if ($PSCmdlet.ParameterSetName -eq 'ByBlueprintId') {
                ...
                $uri = "/v1.0/servicePrincipals/microsoft.graph.agentIdentity?`$filter=agentIdentityBlueprintId eq '$AgentIdentityBlueprintId'"
                $allAgentIdentities = @()
                ...
                foreach ($agentIdentity in $allAgentIdentities) {
                    ...
                    if ($Force -or $PSCmdlet.ShouldProcess("Agent Identity '$displayName' ($agentIdValue) and its Agent Users", "Delete")) {
                        ...
                        $userUri = "/v1.0/users/microsoft.graph.agentUser?`$filter=identityParentId eq '$agentIdValue'"
                        try {
                            ...
                            foreach ($user in $agentUsers) {
                                try {
                                    ...
                                    $deleteUserUri = "/v1.0/users/$($user.id)"
                        ...
                        $deleteUri = "/v1.0/servicePrincipals/$agentIdValue"
                ...
                $getUri = "/v1.0/servicePrincipals/microsoft.graph.agentIdentity/$AgentId"
                ...
                if ($Force -or $PSCmdlet.ShouldProcess("Agent Identity '$displayName' ($AgentId) and its Agent Users", "Delete")) {
                    ...
                    $userUri = "/v1.0/users/microsoft.graph.agentUser?`$filter=identityParentId eq '$AgentId'"
                    try {
                        $userResponse = Invoke-MgGraphRequest -Method GET -Uri $userUri -ErrorAction Stop
                        $agentUsers = @()
                        if ($userResponse -and $userResponse.value) {
                    ...
                    $deleteUri = "/v1.0/servicePrincipals/$AgentId"
                    Invoke-MgGraphRequest -Method DELETE -Uri $deleteUri -ErrorAction Stop
